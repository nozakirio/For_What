...
RSpec.describe AdminBook, type: :model do
  describe 'バリデーションのテスト' do
    ...
    it "全て入力してあるので保存される" do
      admin_book = FactoryBot.create(:admin_book, user_id: @user.id, book_id: @book.id, post_id: @post.id)
      expect(admin_book).to be_valid
    end
    
    it "have_readがない場合、無効"  do
      admin_book = AdminBook.new(have_read: nil)
      expect(admin_book).to_not be_valid
    end
    
    it "want_readがない場合、無効"  do
